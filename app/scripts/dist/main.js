  ...
  _createClass(FormHandler, [{
    ...
    value: function addSubmitHandler(fn) {
      ...
      this.$formElement.on('submit', function (event) {
        ...
        var data = {};
        (0, _jquery2.default)(_this.$formElement).serializeArray().forEach(function (item) {
          data[item.name] = item.value;
          console.log(item.name + ' is ' + item.value);
        });
        console.log(data);
        fn(data).then(function () {
          _this.$formElement[0].reset();
          _this.$formElement[0].elements[0].focus();
        });
      });
    }
  }]);
